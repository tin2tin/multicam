...
class SEQUENCER_HT_header(Header):
    ...
    def draw(self, context):
        ...
        if st.view_type == 'PREVIEW' and strip.type == "MULTICAM" and strip_channel > 2:
            layout.separator_spacer()       
            col = layout.column()
            row = col.row(align=True)
            # Show the selected by disabling it.
            if st.display_channel != 0:
                sub = row.row(align=True)
                if st.display_channel == strip.multicam_source:
                    sub.enabled = False
                else:
                    sub.enabled = True
                sub.operator("sequencer.split_multicam", text="Switch").camera = int(st.display_channel)
                row.prop(st, "display_channel", text="")
            else:
                row.label(text="Switch")
